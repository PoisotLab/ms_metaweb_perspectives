...
> also want to express concern (which we will do more explicitely in the
...
> address them in depth in the revision.
...
> areas that were already in the *original submission*; we would think that a
> description of what the *current submissions* includes would better be infered
> from the abstract. Furthermore, we are unsure about what the intent of this
> comment is. As the reviewer has not formulated actionnable critiques about the
> manuscript (such as, for example, the identification of areas where a better
> description of what the manuscript brings), no changes were made.
...
> categories of embeddings to the main text.
...
> We fail to see how the text is misleading. No changes were made.
...
> and more suitable for a review.
...
> difficult for us to decide what the reviewer is actually discussing.
...
> the previous round, and has no actionnable items related to the *current
...
> embedding, and would greatly appreciate if the reviewer could refrain from
> base accusations against our level of scientific interest.
...
> predictors to learn the embedding. We have changed the title of the figure.
...
> This statement is egregious and demonstrably false. The ideas contained in
> this paragraph have no relationship to our previous work, and this should be
> obvious by examining the literature cited. The fragments of text cherry-picked
> by the reviewer are not ideas we claim credit for; they are important notions
> for practitioners of ML to consider at all times. No changes made to the
> manuscript.
...
> text of the actual manuscript have adequately adressed it.
...
> specifically by adressing the status of documents v. undocumented
...
> the *mebdding* is learned by the transfer model, and therefore the entire
> figure is dealin with embedded networks. No changes made.
...
> description of the case study, and futher clarified this point by presenting
...
> points by reviewer 5, which resulted in new information.
...
> expanded the section on the use of node metadata to calrify which hypotheses
> are associated to broad families of predictors, alongside references to
> published litterature showing that importance of these predictors. We would
...
> beforehands. We have added this information in the section of embeddings, but
...
> interaction networks data. We also touch upon the issue of co-occurence in the
> illustration, and have added references highlight why co-occurrence alone is
> not a good predictor to the introduction.
...
> This is corect, and we addressed this point in the original submission, in the
> next paragraph (which is focused on backbones of networks). No changes were
> made.
...
> As we have mentionned in other places in the manuscript, this is
> context-dependant, and existing litterature suggests that one should explore
...
> importanty feature of embeddings is that they do not assume a specific
...
> We have adressed this comment by adding a paragraph about the status of latent
> values w.r.t. response/effect traits, which we can summarize as "the latent
> values should not be discussed as functional traits". Furthermore, we have
> added a justification of why RPDG is an interesting embedding technique in the
> first paragraph of the section on the illustration.