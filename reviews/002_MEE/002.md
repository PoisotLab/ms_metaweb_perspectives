...
significantly "offer conceptual advances or opinions or identity gaps". I'll now
...
1) Fig.1 is a good diagram that shows the whole pipeline with the input graph
   adjacency matrix, output graph embedding and combined with transfer learning
   technique. I would also suggest the authors to include some experimental
   results based on graph embedding and transfer learning for specie interaction
   inference with real dataset.

2) Please update the article information for the paper (Xu, M.. Understanding
   graph embedding methods and their applications. SIAM Review, 2021) in the
   reference section.
